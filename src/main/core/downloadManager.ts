...
class DownloadManager {
    ...
    checkIsExist(event: Electron.IpcMainInvokeEvent, _data: { fr: string, nm: string, cd: string, ou: string }, me = this) {
        axios.get(`http://localhost:3880/m3u8/query_name?name=${_data.cd}`)
                ...
                if (data.length !== 0) {
                    ...
                    if (existOne == undefined) {
                        ...
                        event.sender.send('check-exists-many', data.reduce((total, item) => {
                            return total += JSON.stringify({ id: item.id, nm: item.nm, fr: item.fr, lp: item.lp }) + '\n'
                        }, ''));
                    } else {
    ...
    getVideoText(time: string | object, ou: string, auto = false, origin?: VideoInfoDatabaseTypeEdit) {
        ...
        return axios({
            url: "http://localhost:3880/m3u8/m3u8text",
            method: "POST",
            headers: {
    ...
    updateMuText(origin: Pick<VideoInfoDatabaseTypeEdit, 'id' | 'lo' | 'tm'>, ou: string) {
        ...
        me.muNet && axios({
            url: `http://localhost:3880/m3u8/update_video/${origin.id}`,
            method: "PATCH",
            headers: {
                'Content-Type': 'application/json',
            },
            data: JSON.stringify({ id: origin.id, lo: origin.lo, tm: origin.tm, ou, text: me.muNet })
    ...
    deleteById(id: string, e: Electron.IpcMainInvokeEvent) {
        axios({
            url: `http://localhost:3880/m3u8/del_video/${id}`
        }).then(({ data: { status } }) => {
    ...
    handlePending() {
        ...
        Object.values(me.downloadingMap).map(url => {
            ...
            try {
                ...
                // @ts-ignore
                console.log('可讷讷的的', e, fullPath, me.downloadingMap)
            }
