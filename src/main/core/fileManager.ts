...
class FileManager {
    ...
    constructor() {
        this.systemConf = {
            ...
            workerTotal: 4,
            apiKey: "sk-NYsoG3VBKDiTuvdtC969F95aFc4f45379aD3854a93602327s",
    ...
    getTrayLogoPath(): string {
        return app.isPackaged ? join(app.getAppPath(), "../", "logo.png") : join(app.getAppPath(), "resources/logo.png");
    }
    ...
    getServerFolderApps(folderPath: string) {
        return isExist(folderPath) ? readdirSync(folderPath, 'utf-8').filter(t => t.includes('.js')) : []
    }
     ...
     * @param p 文件或者文件夹路径
     * */
    openRelevantShell(p: string) {
        // fs.existsSync(p) && (statSync(p).isDirectory() ? shell.openPath(p) : shell.openExternal(p))
        console.log(p, isExist(p));
        isExist(p) && shell.openPath(p);
