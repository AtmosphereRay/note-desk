...
contextBridge.exposeInMainWorld('_tool', {
    ...
    initConf(data: InitConf, insertEl: string = "", els: string[] = []) {
        const name = data.nm.replace(/[\p{P}\p{S}]/ug, '');
        ...
        if (name.match(/fc2.*?(\d{4,})/gi)) {
            const fc2 = name.match(/\d{4,}/)[0];
            globalConf.cd = 'fc2-' + fc2;
        }
...
ipcRenderer.on('deleteSuccess', () => {
    document.querySelector('#del_v') && document.querySelector('#del_v').remove();
    document.querySelector('#my_udp') && document.querySelector('#my_udp').remove()
})
...
function checkIsExist() {
    ...
    timer = setInterval(() => {
        if (globalConf.ou && cache.isExist === false) {
            setTimeout(() => {
                ipcRenderer.invoke('downloadHandle', JSON.stringify({
                    eventName: "check-is-exist", conf: {
                        fr: globalConf.fr,
                        nm: name,
                        cd: globalConf.cd
                    }
...
function createButton(options) {
    const {
        ...
        bgColor = 'transparent',
        textContent,
        parentElem = document.querySelector('#handle'),
        clickHandler, customAttrs, ctx, id, disabled
    } = options;
    const btn = document.createElement('button');   // 创建按钮元素
    btn.style.width = width + 'px'; // 设置元素的样式和内容
    btn.style.height = height + 'px';
